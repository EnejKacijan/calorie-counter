...
createServer(async (request, response) => {
  try {
    ...
    if (url.pathname === "/api/foods/search" && request.method === "GET") {
      return sendJson(response, 200, { foods: await searchFoods(url.searchParams.get("q") || "") });
    }
    ...
    console.error(error);
    return sendJson(response, 500, { error: error.message || "Something went wrong on the server." });
  }
}).listen(port, host, () => {
...
async function searchFoods(query) {
  ...
  const [usdaFoods, offFoods] = await Promise.allSettled([searchUsda(cleanQuery), searchOpenFoodFacts(cleanQuery)]);
  const combined = [
    ...localMatches,
